...
document.querySelector(".vaciarTabla").addEventListener("click", function () {
  if (localStorage.getItem("listaPokemon")) {
    ...
    document.querySelector(".datosPokemon").classList.remove("active");
  ...
  function mostrarEnTable() {
    ...
    arrayPokemons.forEach((poke) => {
      ...
      tr.appendChild(
        crearElemento(
          "td",
          poke.tipos.map((tipo) => tipo.type.name).join(", "),
          "pokeTipos"
        )
      );
      ...
      tr.appendChild(
        crearElemento(
          "td",
          poke.habilidades.map((habilidad) => habilidad.ability.name).join(", "),
          "pokeHabilidades"
        )
      );
