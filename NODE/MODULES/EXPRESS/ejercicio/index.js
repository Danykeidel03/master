...
app.get("/api", (request, response) => {
  ...
  response
    .status(200)
    .json({ success: "OK", mensaje: `Hola ${nombre}, edad ${edad}` });
});
...
app.post("/registro", (request, response) => {
  ...
  response
    ...
    .json({
      success: "OK",
      mensaje: `Usuario creado, Bienvenido ${nombre} con mail ${mail}`,
    });
...
app.post("/actualizar", (request, response) => {
  ...
  response
    .status(200)
    .json({ success: "OK", mensaje: `Hola ${nombre}, hemos cambiado tu mail` });
  response.end();
});
...
app.get("/users", (request, response) => {
  response
    .status(200)
    .json({ success: "OK", mensaje: `Bien hecho`, data: users });
});
